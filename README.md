...
Project sudah memiliki `vercel.json` di root agar Vercel membangun aplikasi React/Vite dari folder `frontend` dan mengarahkan semua route React Router ke `index.html`.

> Catatan: Vercel digunakan untuk frontend. Backend Express + Prisma tetap perlu dideploy terpisah, misalnya ke Railway atau Render, lalu URL backend production dimasukkan ke environment variable Vercel.

Environment variable yang perlu diisi di Vercel:

```env
VITE_APP_NAME=PsyScreening
...
3. Import repository PsyScreening.
4. Pastikan Vercel membaca konfigurasi dari `vercel.json`.
5. Tambahkan environment variable di menu `Settings > Environment Variables`.
6. Klik `Deploy`.
...
- `remote`: memanggil API AI/ML FastAPI melalui backend Express

API AI/ML yang digunakan saat ini:

- Base URL: `https://fastapi-psyscreening-production.up.railway.app`
- Endpoint: `POST /predict`
- Swagger: `https://fastapi-psyscreening-production.up.railway.app/docs`

Swagger API tersebut hanya menyediakan endpoint health dan prediksi, belum menyediakan endpoint chatbot/conversation. Karena itu fitur chat screening PsyScreening tetap menggunakan guided conversation internal. Setelah seluruh jawaban terkumpul, backend mengirim data profil dan jawaban screening ke endpoint `/predict`, lalu menyimpan hasilnya ke database.

Tambahkan konfigurasi berikut di `backend/.env` untuk memakai API AI/ML Railway:
...
Jika `ML_PROVIDER=mock`, sistem tetap memakai model mock internal. Mode ini aman untuk development lokal saat API eksternal sedang tidak dapat diakses.

Field profil yang perlu dilengkapi user sebelum submit analisis AI/ML remote:
...
Response mentah AI/ML disimpan di `ScreeningResult.aiRawResponse`, sedangkan nilai utama dinormalisasi ke field lama seperti `riskLevel`, `score`, `modelUsed`, `insights`, dan `recommendations` agar halaman hasil dan riwayat tetap kompatibel.
